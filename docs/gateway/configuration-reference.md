...
- `model`: string form overrides `primary` only; object form `{ primary, fallbacks }` overrides both (`[]` disables global fallbacks). Agents with a dedicated `primary` no longer implicitly fall back to the global primary whenever the defaults also define global fallbacks but the agent does not define its own `fallbacks`, including the case where `agents.defaults.model.primary` is unset; set `fallbacks` explicitly if you want that behavior. Cron jobs that only override `primary` still inherit default fallbacks unless you set `fallbacks: []`.
