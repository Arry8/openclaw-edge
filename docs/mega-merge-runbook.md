...
Operational reference for running `scripts/mega-merge.ts` — the script that drives the daily integration build.
...
| `--auto-skip-on-build-failure` | off | On build failure: find culprit PR via `git log --first-parent`, revert it, retry build. Records auto-skips in `docs/mega-merge-autoskip.json`. Sends ntfy.sh alert if it cannot auto-fix. |
...
The daily run is defined in [`.github/workflows/mega-merge.yml`](../.github/workflows/mega-merge.yml). It runs at 08:00 UTC with a 60-minute timeout. Manual trigger available via `workflow_dispatch` with optional `limit` and `skip_build` inputs.
