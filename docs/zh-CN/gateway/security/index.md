...
> [!WARNING]
> **个人助手信任模型：** 本指南假设每个网关有一个可信操作者边界（单用户/个人助手模型）。
> OpenClaw **不是** 为多个互不信任的用户共享同一个 agent/网关而设计的主机多租户安全边界。
> 如果需要混合信任或对抗性用户操作，请拆分信任边界（单独的网关 + 凭证，理想情况下使用单独的 OS 用户/主机）。

## 范围优先：个人助手安全模型

OpenClaw 安全指南假设一种**个人助手**部署模式：一个可信操作者边界，可能有多个 agent。

- 支持的安全姿态：每个网关一个用户/信任边界（建议每个边界使用一个 OS 用户/主机/VPS）。
- 不支持的安全边界：多个互不信任或对抗性用户共享同一个网关/agent。
- 如果需要对抗性用户隔离，请按信任边界拆分（单独的网关 + 凭证，理想情况下使用单独的 OS 用户/主机）。
- 如果多个不可信用户可以向同一个启用了工具的 agent 发消息，则将他们视为共享该 agent 的相同委托工具权限。

本页面解释**在该模型内**的加固方法。它不声称在一个共享网关上提供对抗性多租户隔离。
...
- 如果有人可以修改网关主机状态/配置（`~/.openclaw`，包括 `openclaw.json`），则将其视为可信操作者。
- 为多个互不信任/对抗性操作者运行一个网关**不是推荐的设置**。
- 对于混合信任的团队，请使用单独的网关拆分信任边界（或至少使用单独的 OS 用户/主机）。
- OpenClaw 可以在一台机器上运行多个网关实例，但推荐操作倾向于清晰的信任边界分离。
- 推荐默认值：每台机器/主机（或 VPS）一个用户，该用户一个网关，该网关中一个或多个 agent。
- 如果多个用户需要使用 OpenClaw，请为每个用户使用一台 VPS/主机。

### 实际后果（操作者信任边界）

在一个网关实例内部，经过认证的操作者访问是可信的控制平面角色，而不是按用户租户角色。

- 具有读取/控制平面访问权限的操作者可以按设计检查网关会话元数据/历史记录。
- 会话标识符（`sessionKey`、会话 ID、标签）是路由选择器，而不是授权令牌。
- 例如：期望对 `sessions.list`、`sessions.preview` 或 `chat.history` 等方法进行按操作者隔离超出了此模型。
- 如果需要对抗性用户隔离，请按信任边界运行单独的网关。
- 一台机器上的多个网关在技术上可行，但不是多用户隔离的推荐基线。

## 个人助手模型（非多租户总线）

OpenClaw 设计为个人助手安全模型：一个可信操作者边界，可能有多个 agent。

- 如果多个人可以向同一个启用了工具的 agent 发消息，他们每个人都可以操控相同的权限集。
- 按用户的会话/记忆隔离有助于保护隐私，但不会将共享 agent 转换为按用户的主机授权。
- 如果用户之间可能存在对抗性，请按信任边界运行单独的网关（或单独的 OS 用户/主机）。

### 共享 Slack 工作区：真实风险

如果"Slack 中的每个人都可以给机器人发消息"，核心风险是委托工具权限：

- 任何被允许的发送者都可以在 agent 策略范围内引发工具调用（`exec`、浏览器、网络/文件工具）；
- 来自一个发送者的提示词/内容注入可以导致影响共享状态、设备或输出的操作；
- 如果一个共享 agent 拥有敏感凭证/文件，任何被允许的发送者都可能通过工具使用驱动数据外泄。

为团队工作流使用具有最少工具的单独 agent/网关；将个人数据 agent 保持私密。

### 公司共享 agent：可接受的模式

当使用该 agent 的每个人都在同一信任边界内（例如一个公司团队）且 agent 严格限定于业务范围时，这是可接受的。

- 在专用机器/VM/容器上运行；
- 为该运行时使用专用 OS 用户 + 专用浏览器/配置文件/账户；
- 不要将该运行时登录到个人 Apple/Google 账户或个人密码管理器/浏览器配置文件。

如果在同一运行时上混合个人和公司身份，就会消除隔离并增加个人数据暴露风险。

## 网关与节点信任概念

将网关和节点视为一个操作者信任域，但具有不同的角色：

- **网关**是控制平面和策略面（`gateway.auth`、工具策略、路由）。
- **节点**是配对到该网关的远程执行面（命令、设备操作、主机本地能力）。
- 经过认证调用网关的调用者在网关范围内是受信任的。配对后，节点操作是该节点上的受信任操作者操作。
- `sessionKey` 是路由/上下文选择，而不是按用户认证。
- Exec 审批（白名单 + 询问）是操作者意图的护栏，而不是对抗性多租户隔离。
- Exec 审批绑定精确的请求上下文并尽力直接本地文件操作数；它们不会在语义上建模每个运行时/解释器加载器路径。使用沙箱和主机隔离来实现强边界。

如果需要对抗性用户隔离，请按 OS 用户/主机拆分信任边界并运行单独的网关。
...
| 边界或控制                           | 含义                         | 常见误读                                         |
| ------------------------------------ | ---------------------------- | ------------------------------------------------ |
| `gateway.auth`（令牌/密码/设备认证） | 验证调用者到网关 API 的身份  | "需要对每一帧进行消息签名才能安全"               |
| `sessionKey`                         | 用于上下文/会话选择的路由键  | "会话密钥是用户认证边界"                         |
| 提示词/内容护栏                      | 降低模型滥用风险             | "仅提示词注入就证明认证绕过"                     |
| `canvas.eval` / 浏览器 evaluate      | 启用时的有意操作者能力       | "任何 JS eval 原语在此信任模型中自动成为漏洞"    |
| 本地 TUI `!` shell                   | 显式的操作者触发的本地执行   | "本地 shell 便捷命令是远程注入"                  |
| 节点配对和节点命令                   | 配对设备上的操作者级远程执行 | "远程设备控制在默认情况下应被视为不可信用户访问" |

## 非设计漏洞

以下模式经常被报告，除非展示了真实的边界绕过，否则通常会以无需操作关闭：

- 仅提示词注入链，没有策略/认证/沙箱绕过。
- 假设在共享主机/配置上进行对抗性多租户操作的声明。
- 将正常操作者读取路径访问（例如 `sessions.list`/`sessions.preview`/`chat.history`）归类为共享网关设置中的 IDOR 的声明。
- 仅限本地主机的部署发现（例如仅回环网关上的 HSTS）。
- 本仓库中不存在入站路径的 Discord 入站 Webhook 签名发现。
...
2. 报告包含精确代码路径（`file`、函数、行范围）和测试版本/提交。
3. 影响跨越文档化的信任边界（不仅仅是提示词注入）。
4. 声明未列在[超出范围](https://github.com/openclaw/openclaw/blob/main/SECURITY.md#out-of-scope)中。
5. 已检查现有公告以避免重复（适用时复用规范 GHSA）。
6. 部署假设是明确的（回环/本地 vs 暴露，可信 vs 不可信操作者）。

## 60 秒加固基线

首先使用此基线，然后按可信 agent 选择性地重新启用工具：
...
这使网关保持仅限本地访问，隔离私信，并默认禁用控制平面/运行时工具。
