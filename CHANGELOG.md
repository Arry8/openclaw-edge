...
- Plugins/hooks: add a blocking `pre_route` typed hook that can override route/session for the current inbound turn, enabling dynamic route provisioning to apply on the first inbound message instead of waiting for a follow-up turn.
...
- Telegram/polling: quarantine a repeatedly replayed `update_id` after four consecutive deliveries in the same gateway process, log the recovery path, and advance the persisted offset so a poisoned update cannot trap polling in a replay storm.
...
- Cron/webhook SSRF policy: add `cron.webhookSsrfPolicy.dangerouslyAllowPrivateNetwork` and `cron.webhookSsrfPolicy.allowedHostnames` config options so self-hosted deployments can deliver cron webhooks to intranet endpoints that resolve to private IP addresses.
...
- Gateway/node session subscriptions: forward child-session node events through `spawnedBy` lineage when only the parent session is subscribed, so spawned agent chat events no longer disappear from gateway node subscribers. (#34905) Thanks @BryanTegomoh.
