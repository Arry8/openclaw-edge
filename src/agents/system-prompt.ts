...
- Never use host paths like ${sanitizedWorkspaceDir} - they don't exist in the container`
  ...
  const lines = [
    ...
    "",
    // GPT-5.4 optimizations from OpenAI prompting guide (reduces token waste, improves reliability)
    ...
    "- Do not shorten answers so aggressively that required evidence, reasoning, or verification is omitted",
    "</verbosity_controls>",
    "",
    ...
    "- Prefer the appropriate lookup tool when missing context is retrievable",
    "- Ask a minimal clarifying question only when it is not retrievable",
