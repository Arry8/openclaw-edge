...
export const FIELD_HELP: Record<string, string> = {
  ...
  "gateway.auth.scopes":
    "Scopes granted to device-less connections that authenticate via shared token or password. Without this, device-less connections receive no scopes after clearUnboundScopes. Include operator.pairing to enable auto-pairing for remote agents.",
  ...
  "tools.exec.denylist":
    "Command denylist patterns that always block matching exec commands, even when allowlist or full-security mode would otherwise permit them.",
  ...
  "session.injectOutboundToTargetSession":
    'When true and dmScope is an isolated mode (per-peer, per-channel-peer, per-account-channel-peer), outbound messages sent via the message tool are also recorded as assistant messages in the target user\'s session transcript. This preserves conversation context so the target user\'s next reply includes the sent message. Default: false.',
  ...
  "hooks.mappings[].channel":
    'Delivery channel override for mapping outputs (for example "last", "telegram", "slack", "feishu", or any registered plugin channel). Keep channel overrides explicit to avoid accidental cross-channel sends.',
  "hooks.mappings[].to":
