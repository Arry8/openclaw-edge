...
- Full merge history in `edge-merge/docs/history.json`
- Changelog in `edge-merge/docs/changelog.md`
...
| `--auto-skip-on-build-failure` | off | On build failure: parse error output, find culprit PR via `git log --first-parent`, revert it, retry build. Records auto-skips in `edge-merge/docs/autoskip.json`. Sends ntfy.sh alert if it cannot auto-fix. |
